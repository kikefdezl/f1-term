...
fn session_info(value: SessionInfoPayload, quali_part: Option<u8>) -> Result<SessionInfo> {
    ...
    let archive_status = match value.ArchiveStatus.Status.as_str() {
        ...
        _ => {
            return Err(format!("Unknown ArchiveStatus: {}", value.ArchiveStatus.Status).into());
        }
    };
...
pub fn parse_session_info(info_val: &Value, data_val: Option<&Value>) -> Result<SessionInfo> {
    let quali_part = data_val.and_then(crate::parsing::session_data::qualifying_part);

    match info_val {
        Value::Object(_) => match SessionInfoPayload::deserialize(info_val) {
            Ok(sip) => session_info(sip, quali_part),
